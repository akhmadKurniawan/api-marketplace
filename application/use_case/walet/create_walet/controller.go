...
func (h *CreateWaletHandler) CreateWalet(c *gin.Context) {
	...
	if err := c.ShouldBind(&req); err != nil {
		log.Println("Controller - CreateWalet error while binding request : ", err)
		c.JSON(500, response.SetMessage(err.Error(), false))
		return
	}

	if ok, err := ValidateRequest(&req); !ok {
		log.Println("Controller - CreateWalet error validation : ", err)
		c.JSON(500, response.SetMessage(err.Error(), false))
		return
	}
	...
	if errCreate != nil {
		log.Println("Controller - CreateWalet error while access service : ", errCreate)
